...
import os
import requests
from lib import SupportBee, SupportBeeException
...
class InstallController(CementBaseController):
    ...
    def install(self):
        ...
        app_settings['web'] = {}
        app_settings['rethink']['db_name'] = click.prompt("Database Name", default="supportbee")
        app_settings['rethink']['db_host'] = click.prompt("Database Host", default="localhost")
        app_settings['rethink']['db_port'] = click.prompt("Database Port", default="28015")
        try:
            r_conn = r.connect(host=app_settings['rethink']['db_host'], port=int(app_settings['rethink']['db_port']))
        ...
        try:
            ...
            print(error)
            try:
                click.confirm("Database already exists. Do you want to continue?", abort=True)
            except click.exceptions.Abort:
                print("Exiting...")
                exit()
        try:
            r.db(app_settings['rethink']['db_name']).table_create("tickets").run(r_conn)
            r.db(app_settings['rethink']['db_name']).table_create("replies").run(r_conn)
            r.db(app_settings['rethink']['db_name']).table_create("teams").run(r_conn)
            r.db(app_settings['rethink']['db_name']).table_create("users").run(r_conn)
            r.db(app_settings['rethink']['db_name']).table_create("labels").run(r_conn)
        except RqlRuntimeError as error:
            print(error)
            try:
                click.confirm("Tickets table already exists. Do you want to continue?", abort=True)
            except click.exceptions.Abort:
                print("Exiting...")
                exit()
        app_settings['web']['port'] = click.prompt("On which port do you want to run this app?")
        app_settings['supportbee']['company'] = click.prompt("SupportBee Company (?.supportbee.com)")
        app_settings['supportbee']['apikey'] = click.prompt("SupportBee API Key")
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
        base_url = "https://" + app_settings['supportbee']['company'] + ".supportbee.com"
        url_params = {"auth_token": app_settings['supportbee']['apikey']}
        check_request = requests.get(base_url + "/users", params=url_params, headers=headers)
        if check_request.status_code == 200:
            print("Authenticated.")
        else:
            print(check_request.status_code)
            print("Non-200 status received from SupportBee. Please check your API key and company name...")
            exit()
...
class ExcelController(CementBaseController):
    ...
    def default(self):
        ...
        if not os.path.isfile('./config.ini'):
            print("The settings file does not exists. Please run 'python support_cli.py install'")
        else:
            print("Initializing...")
            app_settings = configparser.ConfigParser()
    ...
    def excel_report(self):
        filename = click.prompt("What do you want to name the file (before the .xlsx)?", default="myfile")
...
class SupportBaseController(CementBaseController):
    ...
    def sync_tickets(self):
        ...
        for ticket in ticket_response['tickets']:
            ...
            print("Found " + str(len(replies)) + " replies. Writing the replies")
